...
#+begin_src python :tangle /tmp/short.py
  from random import choice, seed
  from tadashi.apps import Simple
  seed(42)
  app = Simple(f"/home/vatai/code/tadashi/examples/inputs/depnodep.c")
  ...
  print(f"speedup= {app.measure()/tapp.measure()}x")
#+end_src
** Managing states
:PROPERTIES:
:beamer_env: fullframe
...
** ~app.transform_measure_mpi(trs_list, self.executor)~
*** Left column
:PROPERTIES:
:beamer_col: 0.6
:END:
...
- Gather the speedups
- Uses ~MPI4py~, and exposed to the user as ~concurrent.futures~ (from the
  Python standard)
*** Right column
:PROPERTIES:
:beamer_col: 0.3
:END:
file:figs/harness.png
** Two Backends: PET and Polly
*** Left column
:PROPERTIES:
:beamer_col: 0.6
:END:
\includegraphics[width=\textwidth]{./figs/translator-pet.mmd.pdf}
\includegraphics[width=\textwidth]{./figs/translator-polly.mmd.pdf}
...
#+begin_example
  clang -O0 -Xclang -disable-O0-optnone \
    -c -emit-llvm src.c -o src.O0.bc
  opt -polly-canonicalize \
    src.O0.bc -o=src.pre_polly.bc
  opt -aa-pipeline=basic-aa \
    -polly-codegen src.pre_polly.bc \
    -polly-export-jscop -o=/dev/null
  # ISL/Tadashi
  opt -aa-pipeline=basic-aa \
    -polly-codegen src.pre_polly.bc \
    -polly-import-jscop -o=/dev/null
  opt -O3 src.post_polly.bc -o=src.bc
  llc -O3 -relocation-model=pic --filetype=obj \
    src.bc -o=tsrc.o
#+end_example

** WIP Roadmap
- Expose tadashi through MCP to agents
- More ML!
- Better polyhedral extraction
  - MLIR
  - Linearization for PET
  - beyond polyhedral
- Better codegen
  - MLIR
  - CUDA support
